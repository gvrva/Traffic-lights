...
Первой архитектурой, выбранной для предсказания, была FasterRCNN. Обученные версии этой модели классифицировали картинку на 2 класса: светофор и фон, не указывая при этом сигнал светофора, поэтому дополнительно была разработана функция, определяющая цвет детектированного светофора. Тестировались несколько вариантов алгоритмов определения цвета.
...
При разработке функции check_similarity() сразу была выбрана используемая метрика, тестировались только пороговые значения. Оптимальным в ходе экспериментов оказалось значени 0.02.
Для функции affect были протестированы 3 алгоритма.
1. Выбор самого центрального светофора. Данный алгоритм часто выдавал неверные результаты, так как самы центральным мог оказаться дальний светофор, не влияющий в действительности на движение машины в данный момент.
